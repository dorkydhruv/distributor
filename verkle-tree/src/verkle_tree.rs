...
use ark_serialize::CanonicalSerialize; // ensure crate in Cargo.toml
...
impl VerkleTree {
    pub fn new(leaves: &[F]) -> Result<Self, VerkleTreeError> {
        if leaves.is_empty() { return Err(VerkleTreeError::EmptyInput); }
        ...
        let mut current: Vec<VerkleNode> = leaves
            ...
            .map(|chunk| {
                ...
                VerkleNode { commitment: com, polynomial: poly, children: None, raw_leaf_count: chunk_vec.len() }
            })
            .collect();
        while current.len() > 1 {
        ...
        Ok(Self { total_leaves: leaves.len(), root, kzg })
    }
    ...
    fn parent_layer(kzg: &KZGCommitment, layer: Vec<VerkleNode>) -> Vec<VerkleNode> {
        layer
            .chunks(VERKLE_TREE_WIDTH)
            .map(|group| {
                let child_values: Vec<F> = group.iter().map(|c| hash_commitment_to_field(&c.commitment)).collect();
                let poly = KZGCommitment::vector_to_polynomial(&child_values);
                let com = kzg.commit_polynomial(&poly);
                let raw_leaf_count = group.iter().map(|c| c.raw_leaf_count).sum();
                VerkleNode { commitment: com, polynomial: poly, children: Some(group.to_vec()), raw_leaf_count }
            })
            .collect()
    }

    pub fn root_commitment(&self) -> G1Affine { self.root.commitment }
    pub fn leaf_count(&self) -> usize { self.total_leaves }

    pub fn prove(&self, leaf_index: usize, leaf_value: F) -> Result<VerkleProof, VerkleTreeError> {
        if leaf_index >= self.total_leaves { return Err(VerkleTreeError::IndexOutOfRange); }
        ...
        loop {
            match &node.children {
                Some(children) => {
                    ...
                    let mut acc = 0usize; let mut child_idx = 0usize; let mut child_offset = offset;
                    for (i, ch) in children.iter().enumerate() {
                        if offset < acc + ch.raw_leaf_count { child_idx = i; child_offset = offset - acc; break; }
                    ...
                    let proof = self.kzg.generate_proof(&node.polynomial, &proof_points)
                        .map_err(|_| VerkleTreeError::ProofFailure)?;
                    path.push(ProofNode { commitment: node.commitment, eval_index: child_idx as u16, eval_value, kzg_proof: proof });
                ...
                None => {
                    ...
                    let proof = self.kzg.generate_proof(&node.polynomial, &proof_points)
                        .map_err(|_| VerkleTreeError::ProofFailure)?;
                    path.push(ProofNode { commitment: node.commitment, eval_index: offset as u16, eval_value: leaf_value, kzg_proof: proof });
                    break;
                }
            }
        }
        Ok(VerkleProof { leaf_index: leaf_index as u32, leaf_value, path })
    }
}
...
pub fn hash_commitment_to_field(c: &G1Affine) -> F {
    let mut bytes = Vec::with_capacity(48);
        c.serialize_compressed(&mut bytes).expect("serialize commitment");
...
pub fn hash_leaf_bytes_to_field(data: &[u8]) -> F { hash_to_field(b"verkle:leaf", data) }

fn hash_to_field(domain: &[u8], data: &[u8]) -> F {
    // 64-byte wide expansion using two Blake3 hashes with domain separation.
    let mut h1 = Hasher::new(); h1.update(domain); h1.update(data); let r1 = h1.finalize();
    let mut h2 = Hasher::new(); h2.update(domain); h2.update(data); h2.update(&[1]); let r2 = h2.finalize();
    let mut wide = [0u8; 64];
...
pub fn verify_proof(root: &G1Affine, proof: &VerkleProof) -> bool {
    if proof.path.is_empty() { return false; }
    if proof.path[0].commitment != *root { return false; }
    ...
    if !kzg.verify_proof(&pn.commitment, &pts, &pn.kzg_proof) { return false; }
        if i < proof.path.len() - 1 { // internal linkage: eval_value should be hash(child commitment)
            let child_commit = proof.path[i+1].commitment;
            let expected = hash_commitment_to_field(&child_commit);
            if expected != pn.eval_value { return false; }
        } else { // last node (leaf layer) eval_value should equal provided leaf_value
            if pn.eval_value != proof.leaf_value { return false; }
        }
    }
    true
}
...
mod tests {
    ...
    fn build_and_prove_small() {
        ...
        let leaves: Vec<F> = (0u64..100).map(|i| hash_leaf_bytes_to_field(&i.to_le_bytes())).collect();
        let tree = VerkleTree::new(&leaves).unwrap();
        for idx in [0usize, 1, 31, 32, 63, 64, 99] { // sample indices
