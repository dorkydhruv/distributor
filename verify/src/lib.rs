...
use blake3::Hasher;
use kzg::{verify_path_multiproof, PathMultiproof, SrsEval, srs_from_storage, WIDTH};
...
pub fn verify_aggregated(root_bytes: &[u8], proof_bytes: &[u8], expected_leaf: &[u8; 32], srs: &SrsEval) -> bool {
    verify_aggregated_with_error(root_bytes, proof_bytes, expected_leaf, srs).is_ok()
}

/// More diagnostic verification returning a structured error for debugging.
pub fn verify_aggregated_with_error(
    root_bytes: &[u8],
    proof_bytes: &[u8],
    expected_leaf: &[u8; 32],
    srs: &SrsEval,
) -> Result<(), AggregatedVerifyError> {
    ...
    let root_slice = if root_bytes.len() >= 32 {
        &root_bytes[0..32]
    } else {
        return Err(AggregatedVerifyError::RootBytes);
    };
    let root = G1Affine::deserialize_compressed(&mut &root_slice[..])
        .map_err(|_| AggregatedVerifyError::RootBytes)?;
    let proof =
        AggregatedProof::from_bytes(proof_bytes).ok_or(AggregatedVerifyError::ProofEncoding)?;
    if proof.commitments.is_empty() {
        return Err(AggregatedVerifyError::Empty);
    }
    if proof.commitments[0] != root {
        return Err(AggregatedVerifyError::RootMismatch);
    }
    ...
    let path_mp = PathMultiproof {
        commitments: proof.commitments,
        indices: proof.indices,
        values: proof.values,
        d_commit: proof.d_commit,
        y: proof.y,
        h_commit: proof.h_commit,
        w: proof.w,
        sigma: proof.sigma,
    };
    if verify_path_multiproof(&path_mp, srs, expected_leaf_field) {
        Ok(())
    } else {
        Err(AggregatedVerifyError::Pairing)
    }
}
...
/// Construct an SRSEval from raw on-chain stored bytes (g1_lagrange, g2_gen, g2_tau)
pub fn srs_from_account(
    g1_bytes: &[[u8;32]; WIDTH],
    g2_gen: &[u8;64],
    g2_tau: &[u8;64]
) -> Result<SrsEval, AggregatedVerifyError> {
    srs_from_storage(g1_bytes, g2_gen, g2_tau).ok_or(AggregatedVerifyError::ProofEncoding)
}

/// Debug helper (not for production): attempts to segment the proof bytes and returns slice offsets.
#[cfg(feature = "debug-segments")]
pub fn debug_segment_proof(
    proof_bytes: &[u8],
) -> Option<alloc::vec::Vec<(&'static str, core::ops::Range<usize>)>> {
    if proof_bytes.is_empty() {
        return None;
    }
    let mut segments = alloc::vec::Vec::new();
    let mut offset = 0;
    let depth = *proof_bytes.get(offset)? as usize;
    segments.push(("depth", offset..offset + 1));
    offset += 1;
    const G1_COMP: usize = 32;
    for i in 0..depth {
        if offset + G1_COMP > proof_bytes.len() {
            return None;
        }
        segments.push(("commitment", offset..offset + G1_COMP));
        offset += G1_COMP;
        if i == 0 { /* root */ }
    }
    ...
    segments.push(("indices_len", offset..offset + 1));
    let idx_len = *proof_bytes.get(offset)? as usize;
    offset += 1;
    if offset + idx_len > proof_bytes.len() {
        return None;
    }
    segments.push(("indices", offset..offset + idx_len));
    offset += idx_len;
    if offset >= proof_bytes.len() {
        return None;
    }
    segments.push(("values_len", offset..offset + 1));
    let val_len = *proof_bytes.get(offset)? as usize;
    offset += 1;
    for _ in 0..val_len {
        if offset + 32 > proof_bytes.len() {
            return None;
        }
        segments.push(("value", offset..offset + 32));
        offset += 32;
    }
    ...
    segments.push(("d_commit", offset..offset + G1_COMP));
    offset += G1_COMP;
    if offset + G1_COMP > proof_bytes.len() {
        return None;
    }
    segments.push(("h_commit", offset..offset + G1_COMP));
    offset += G1_COMP;
    if offset + G1_COMP > proof_bytes.len() {
        return None;
    }
    segments.push(("sigma", offset..offset + G1_COMP));
    offset += G1_COMP;
    if offset + 32 > proof_bytes.len() {
        return None;
    }
    segments.push(("y", offset..offset + 32));
    offset += 32;
    if offset + 32 > proof_bytes.len() {
        return None;
    }
    segments.push(("w", offset..offset + 32));
    offset += 32;
    if offset != proof_bytes.len() {
        return None;
    }
...
impl AggregatedProof {
    fn from_bytes(data: &[u8]) -> Option<Self> {
        ...
        let mut offset = 0;
        let depth = *data.get(offset)? as usize;
        offset += 1;
        let mut commitments = Vec::with_capacity(depth);
        const G1_COMP: usize = 32; // BN254 compressed size
        for _ in 0..depth {
            if offset + G1_COMP > data.len() {
                return None;
            }
            let mut cur = &data[offset..offset + G1_COMP];
        ...
        let idx_len = *data.get(offset)? as usize;
        offset += 1;
        if idx_len != depth {
            return None;
        }
        if offset + idx_len > data.len() {
            return None;
        }
        let indices = data[offset..offset + idx_len].to_vec();
        offset += idx_len;
        if offset >= data.len() {
            return None;
        }
        let val_len = *data.get(offset)? as usize;
        offset += 1;
        if val_len != depth {
            return None;
        }
        ...
        offset += consumed;
        let mut cur = &data[offset..];
        let w = F::deserialize_compressed(&mut cur).ok()?; // last
        Some(Self {
            commitments,
            indices,
            values,
