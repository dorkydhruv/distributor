...
struct SrsHolder { inited: AtomicBool, srs: UnsafeCell<Option<SrsEval>> }
unsafe impl Sync for SrsHolder {}

static HOLDER: SrsHolder = SrsHolder { inited: AtomicBool::new(false), srs: UnsafeCell::new(None) };
...
pub fn static_srs() -> &'static SrsEval {
    if !HOLDER.inited.load(Ordering::Acquire) {
        ...
        unsafe { *HOLDER.srs.get() = Some(SrsEval::deterministic()); }
