...
impl core::fmt::Display for KzgEvalError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        write!(f, "{:?}", self)
    }
}
impl Debug for KzgEvalError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            KzgEvalError::WidthMismatch => write!(f, "WidthMismatch"),
            KzgEvalError::InvalidIndex => write!(f, "InvalidIndex"),
            KzgEvalError::PointInDomain => write!(f, "PointInDomain"),
            KzgEvalError::DivZero => write!(f, "DivZero"),
            KzgEvalError::PairingFailed => write!(f, "PairingFailed"),
        }
    }
}
...
impl SrsEval {
    pub fn assert_width(&self, w: usize) -> Result<(), KzgEvalError> {
        if self.width != w {
            Err(KzgEvalError::WidthMismatch)
        } else {
            Ok(())
        }
    }
    ...
    pub fn deterministic() -> Self {
        ...
        for i in 0..width {
            g1_lagrange[i] = (G1Affine::generator().into_group()
                * lagrange_at(i, tau, &omega_domain))
            .into_affine();
        }
        SrsEval {
            width,
    ...
    /// Export the minimal verification SRS (Lagrange G1 points, g2_gen, g2_tau) into provided buffers.
    /// Expects g1_buf.len() == 32 and each element 32 bytes, g2_gen 64 bytes, g2_tau 64 bytes.
    pub fn export(&self, g1_out: &mut [[u8;32]; crate::WIDTH], g2_gen_out: &mut [u8;64], g2_tau_out: &mut [u8;64]) {
        for (i,p) in self.g1_lagrange.iter().enumerate() { let mut tmp=[0u8;32]; p.serialize_compressed(&mut tmp[..]).ok(); g1_out[i]=tmp; }
    self.g2_gen.serialize_compressed(&mut g2_gen_out[..]).ok();
    self.g2_tau.serialize_compressed(&mut g2_tau_out[..]).ok();
    }
}
...
pub fn srs_from_storage(g1_lagrange: &[[u8;32]; WIDTH], g2_gen_bytes: &[u8;64], g2_tau_bytes: &[u8;64]) -> Option<SrsEval> {
    ...
    let omega_domain = compute_domain(WIDTH);
    let inv_width = F::from(WIDTH as u64).inverse().unwrap();
    Some(SrsEval { width: WIDTH, g1_lagrange: g1_points, g2_gen, g2_tau, omega_domain, inv_width })
}
...
fn compute_domain(width: usize) -> [F; WIDTH] {
    ...
    for _ in 0..(F::TWO_ADICITY as usize - 5) {
        omega = omega.square();
    }
    let mut cur = F::one();
    for i in 0..width {
...
pub fn inner_quotient(
    ...
    k: usize,
    srs: &SrsEval,
) -> Result<[F; WIDTH], KzgEvalError> {
    if k >= srs.width {
        return Err(KzgEvalError::InvalidIndex);
    }
    let width = srs.width;
    let yk = poly.evals[k];
    ...
    for i in 0..width {
        ...
        let fi = poly.evals[i];
        let omega_i = srs.omega_domain[i];
        let omega_k = srs.omega_domain[k];
        let denom = omega_k - omega_i;
        let inv = denom.inverse().ok_or(KzgEvalError::DivZero)?;
        let omega_i_inv = omega_i.inverse().unwrap();
        q[i] = (fi - yk) * omega_i_inv * inv;
    }
    // q[k] = - Σ ω^{i-k} q[i]
    let omega_k_inv = srs.omega_domain[k].inverse().unwrap();
    let mut acc = F::zero();
    for i in 0..width {
        if i != k {
            let factor = srs.omega_domain[i] * omega_k_inv;
            acc += factor * q[i];
        }
    }
    q[k] = -acc;
...
pub fn evaluate_outside(poly: &EvalPoly, t: F, srs: &SrsEval) -> Result<F, KzgEvalError> {
    for &x in srs.omega_domain.iter().take(srs.width) {
        if x == t {
            return Err(KzgEvalError::PointInDomain);
        }
    }
    let mut acc = F::zero();
    let t_pow_w = t.pow([srs.width as u64]);
    for i in 0..srs.width {
        let xi = srs.omega_domain[i];
        let denom = t - xi;
        let inv = denom.inverse().ok_or(KzgEvalError::DivZero)?;
        acc += poly.evals[i] * xi * inv;
    }
...
fn pairing_check(c: &G1Affine, y: F, z: F, pi: &G1Affine, srs: &SrsEval) -> bool {
    ...
    let rhs_g2 =
        (G2Projective::from(srs.g2_tau) + (G2Projective::from(srs.g2_gen) * (-z))).into_affine();
...
fn hash_field(acc: &mut Hasher, f: &F) {
    let mut bytes = [0u8; 32];
    f.serialize_compressed(&mut bytes[..]).ok();
    acc.update(&bytes);
}
fn hash_g1(acc: &mut Hasher, p: &G1Affine) {
    let mut bytes = [0u8; 48];
    p.serialize_compressed(&mut bytes[..]).ok();
    acc.update(&bytes);
}
...
pub fn build_path_multiproof(
    ...
    path_indices: &[usize],
    srs: &SrsEval,
) -> Result<PathMultiproof, KzgEvalError> {
    let depth = polys.len();
    ...
    for p in polys {
        commitments.push(commit_eval(p, srs));
    }
    for (p, &k) in polys.iter().zip(path_indices) {
        values.push(p.evals[k]);
    }
    // r challenge
    ...
    for (k, v) in path_indices.iter().zip(values.iter()) {
        h.update(&[*k as u8]);
        hash_field(&mut h, v);
    }
    ...
    let mut r_pow = F::one();
    for (poly, &k) in polys.iter().zip(path_indices) {
        let q = inner_quotient(poly, k, srs)?;
        for i in 0..srs.width {
            g[i] += r_pow * q[i];
        }
        r_pow *= r;
    ...
    // t challenge
    let mut ht = Hasher::new();
    hash_field(&mut ht, &r);
    hash_g1(&mut ht, &d_commit);
    let t = F::from_le_bytes_mod_order(ht.finalize().as_bytes());
    // h accumulation
    let mut h_eval = [F::zero(); WIDTH];
    let mut r_pow = F::one();
    for (poly, &k) in polys.iter().zip(path_indices) {
        let denom_inv = (t - srs.omega_domain[k])
            .inverse()
            .ok_or(KzgEvalError::DivZero)?;
        for i in 0..srs.width {
            h_eval[i] += r_pow * poly.evals[i] * denom_inv;
        }
        r_pow *= r;
    }
    let h_poly = EvalPoly { evals: h_eval };
    // Evaluate & open h,g at t
    let y = evaluate_outside(&h_poly, t, srs)?;
    let w = evaluate_outside(&g_poly, t, srs)?;
    let qh_eval = outer_quotient(&h_poly, t, y, srs)?;
    let qg_eval = outer_quotient(&g_poly, t, w, srs)?;
    let qh_poly = EvalPoly { evals: qh_eval };
    let qg_poly = EvalPoly { evals: qg_eval };
    let pi_h = commit_eval(&qh_poly, srs);
    let pi_g = commit_eval(&qg_poly, srs);
    let h_commit = commit_eval(&h_poly, srs);
    let mut hq = Hasher::new();
    hash_g1(&mut hq, &h_commit);
    hash_g1(&mut hq, &d_commit);
    hash_field(&mut hq, &y);
    hash_field(&mut hq, &w);
    let q = F::from_le_bytes_mod_order(hq.finalize().as_bytes());
    let sigma = (G1Projective::from(pi_h) + G1Projective::from(pi_g) * q).into_affine();
    Ok(PathMultiproof {
        commitments,
        indices: path_indices.iter().map(|x| *x as u8).collect(),
        values,
        d_commit,
        y,
        h_commit,
...
pub fn verify_path_multiproof(proof: &PathMultiproof, srs: &SrsEval, expected_leaf: F) -> bool {
    ...
    for (k, v) in proof.indices.iter().zip(proof.values.iter()) {
        h.update(&[*k]);
        hash_field(&mut h, v);
    }
    let r = F::from_le_bytes_mod_order(h.finalize().as_bytes());
    ...
    let mut ht = Hasher::new();
    hash_field(&mut ht, &r);
    hash_g1(&mut ht, &proof.d_commit);
    let t = F::from_le_bytes_mod_order(ht.finalize().as_bytes());
    // Recompute q
    let mut hq = Hasher::new();
    hash_g1(&mut hq, &proof.h_commit);
    hash_g1(&mut hq, &proof.d_commit);
    hash_field(&mut hq, &proof.y);
    hash_field(&mut hq, &proof.w);
    let q = F::from_le_bytes_mod_order(hq.finalize().as_bytes());
    // Basic sanity: last value equals expected leaf
    if *proof.values.last().unwrap() != expected_leaf {
        return false;
    }
    ...
    let gamma =
        (G2Projective::from(srs.g2_tau) + (G2Projective::from(srs.g2_gen) * (-t))).into_affine();
...
pub fn open_domain(
    ...
    k: usize,
    srs: &SrsEval,
) -> Result<(F, G1Affine), KzgEvalError> {
    srs.assert_width(width)?;
    if k >= width {
        return Err(KzgEvalError::InvalidIndex);
    }
    let q_evals = inner_quotient(poly, k, srs)?;
    let q_poly = EvalPoly { evals: q_evals };
    let pi = commit_eval(&q_poly, srs);
    Ok((poly.evals[k], pi))
}
